"""Generated wrapper for Enrockvault Solidity contract."""
...
# constructor for Enrockvault below.
try:
    # both mypy and pylint complain about what we're doing here, but this
    # works just fine, so their messages have been disabled here.
    from . import (  # type: ignore # pylint: disable=import-self
        EnrockvaultValidator,
    )
except ImportError:

    class EnrockvaultValidator(  # type: ignore
        Validator
    ):
...
class BankVaultMethod(ContractMethod): # pylint: disable=invalid-name
    """Various interfaces to the bank_vault method."""

    def __init__(self, elib: MiliDoS, contract_address: str, contract_function: ContractFunction, validator: Validator=None):
        """Persist instance data."""
        super().__init__(elib, contract_address, validator)
        self._underlying_method = contract_function
        self.sign = validator.getSignature("bank_vault")

    def validate_and_normalize_inputs(self, index_0: str)->any:
        """Validate the inputs to the bank_vault method."""
        self.validator.assert_valid(
            method_name='bank_vault',
    ...
    def block_send(self, index_0: str,_valeth:int=0) -> int:
        ...
        try:
            ...
            if 'data' in _t:
                ...
                self._on_receipt_handle("bank_vault", tx_receipt, txHash)
            ...
            print(f"{Bolors.FAIL}Error {er} {Bolors.RESET}: bank_vault")
            message = f"Error {er}: bank_vault"
            self._on_fail("bank_vault", message)
        except ValueError as err:
            if "message" in err.args[0]:
                message = err.args[0]["message"]
                print(f"{Bolors.FAIL}Error Revert {Bolors.RESET}, bank_vault: {message}")
            else:
                message = "Error Revert , Reason: Unknown"
                print(f"{Bolors.FAIL}Error Revert {Bolors.RESET}, bank_vault. Reason: Unknown")
...
class CheckMethod(ContractMethod): # pylint: disable=invalid-name
    """Various interfaces to the check method."""

    def __init__(self, elib: MiliDoS, contract_address: str, contract_function: ContractFunction, validator: Validator=None):
        """Persist instance data."""
        super().__init__(elib, contract_address, validator)
        self._underlying_method = contract_function
        self.sign = validator.getSignature("check")

    def validate_and_normalize_inputs(self, coin: str, user: str)->any:
        """Validate the inputs to the check method."""
        self.validator.assert_valid(
            method_name='check',
            parameter_name='coin',
            argument_value=coin,
    ...
    def block_call(self,coin: str, user: str, debug:bool=False) -> List[int]:
        _fn = self._underlying_method(coin, user)
        returned = _fn.call({
                'from': self._operate
            })
        return [int(element) for element in returned]
    def block_send(self, coin: str, user: str,_valeth:int=0) -> int:
        """Execute underlying contract method via eth_call.
        ...
        try:
            ...
            if 'data' in _t:
                ...
                self._on_receipt_handle("check", tx_receipt, txHash)
            ...
            print(f"{Bolors.FAIL}Error {er} {Bolors.RESET}: check")
            message = f"Error {er}: check"
            self._on_fail("check", message)
        except ValueError as err:
            if "message" in err.args[0]:
                message = err.args[0]["message"]
                print(f"{Bolors.FAIL}Error Revert {Bolors.RESET}, check: {message}")
            else:
                message = "Error Revert , Reason: Unknown"
                print(f"{Bolors.FAIL}Error Revert {Bolors.RESET}, check. Reason: Unknown")

            self._on_fail("check", message)

    def send_transaction(self, coin: str, user: str, tx_params: Optional[TxParams] = None) -> Union[HexBytes, bytes]:
        """Execute underlying contract method via eth_sendTransaction.

        :param tx_params: transaction parameters
        """
        (coin, user) = self.validate_and_normalize_inputs(coin, user)
        tx_params = super().normalize_tx_params(tx_params)
        return self._underlying_method(coin, user).transact(tx_params.as_dict())

    def build_transaction(self, coin: str, user: str, tx_params: Optional[TxParams] = None) -> dict:
        """Construct calldata to be used as input to the method."""
        (coin, user) = self.validate_and_normalize_inputs(coin, user)
        tx_params = super().normalize_tx_params(tx_params)
        return self._underlying_method(coin, user).buildTransaction(tx_params.as_dict())

    def estimate_gas(self, coin: str, user: str, tx_params: Optional[TxParams] = None) -> int:
        """Estimate gas consumption of method call."""
        (coin, user) = self.validate_and_normalize_inputs(coin, user)
        tx_params = super().normalize_tx_params(tx_params)
        return self._underlying_method(coin, user).estimateGas(tx_params.as_dict())
...
class DonateErc20Method(ContractMethod): # pylint: disable=invalid-name
    """Various interfaces to the donate_erc20 method."""

    def __init__(self, elib: MiliDoS, contract_address: str, contract_function: ContractFunction, validator: Validator=None):
        """Persist instance data."""
        super().__init__(elib, contract_address, validator)
        self._underlying_method = contract_function
        self.sign = validator.getSignature("donate_erc20")

    def validate_and_normalize_inputs(self, coin: str, amount: int)->any:
        """Validate the inputs to the donate_erc20 method."""
        self.validator.assert_valid(
            method_name='donate_erc20',
            parameter_name='coin',
            argument_value=coin,
        )
        coin = self.validate_and_checksum_address(coin)
    ...
    def block_send(self, coin: str, amount: int,_valeth:int=0) -> None:
        ...
        try:
            ...
            if 'data' in _t:
                ...
                self._on_receipt_handle("donate_erc20", tx_receipt, txHash)
            ...
            print(f"{Bolors.FAIL}Error {er} {Bolors.RESET}: donate_erc20")
            message = f"Error {er}: donate_erc20"
            self._on_fail("donate_erc20", message)
        except ValueError as err:
            if "message" in err.args[0]:
                message = err.args[0]["message"]
                print(f"{Bolors.FAIL}Error Revert {Bolors.RESET}, donate_erc20: {message}")
            else:
                message = "Error Revert , Reason: Unknown"
                print(f"{Bolors.FAIL}Error Revert {Bolors.RESET}, donate_erc20. Reason: Unknown")
...
class GameRoundMethod(ContractMethod): # pylint: disable=invalid-name
    """Various interfaces to the game_round method."""

    def __init__(self, elib: MiliDoS, contract_address: str, contract_function: ContractFunction, validator: Validator=None):
        """Persist instance data."""
        super().__init__(elib, contract_address)
        self._underlying_method = contract_function
        self.sign = validator.getSignature("game_round")
    ...
    def block_send(self, _valeth:int=0) -> int:
        ...
        try:
            ...
            if 'data' in _t:
                ...
                self._on_receipt_handle("game_round", tx_receipt, txHash)
            ...
            print(f"{Bolors.FAIL}Error {er} {Bolors.RESET}: game_round")
            message = f"Error {er}: game_round"
            self._on_fail("game_round", message)
        except ValueError as err:
            if "message" in err.args[0]:
                message = err.args[0]["message"]
                print(f"{Bolors.FAIL}Error Revert {Bolors.RESET}, game_round: {message}")
            else:
                message = "Error Revert , Reason: Unknown"
                print(f"{Bolors.FAIL}Error Revert {Bolors.RESET}, game_round. Reason: Unknown")
...
class OraclePostResultMethod(ContractMethod): # pylint: disable=invalid-name
    """Various interfaces to the oracle_post_result method."""

    def __init__(self, elib: MiliDoS, contract_address: str, contract_function: ContractFunction, validator: Validator=None):
        """Persist instance data."""
        super().__init__(elib, contract_address, validator)
        self._underlying_method = contract_function
        self.sign = validator.getSignature("oracle_post_result")

    def validate_and_normalize_inputs(self, _round: int, dot: int, _hash: str, total_users: int)->any:
        """Validate the inputs to the oracle_post_result method."""
        self.validator.assert_valid(
            method_name='oracle_post_result',
            parameter_name='round',
            argument_value=_round,
        )
        # safeguard against fractional inputs
        _round = int(_round)
        self.validator.assert_valid(
        ...
        self.validator.assert_valid(
            method_name='oracle_post_result',
            parameter_name='total_users',
            argument_value=total_users,
        )
        return (_round, dot, _hash, total_users)



    def block_send(self, _round: int, dot: int, _hash: str, total_users: int,_valeth:int=0) -> None:
        """Execute underlying contract method via eth_call.

        :param tx_params: transaction parameters
        :returns: the return value of the underlying method.
        """
        _fn = self._underlying_method(_round, dot, _hash, total_users)
        try:
            ...
            if 'data' in _t:
                ...
                self._on_receipt_handle("oracle_post_result", tx_receipt, txHash)
            ...
            print(f"{Bolors.FAIL}Error {er} {Bolors.RESET}: oracle_post_result")
            message = f"Error {er}: oracle_post_result"
            self._on_fail("oracle_post_result", message)
        except ValueError as err:
            if "message" in err.args[0]:
                message = err.args[0]["message"]
                print(f"{Bolors.FAIL}Error Revert {Bolors.RESET}, oracle_post_result: {message}")
            else:
                message = "Error Revert , Reason: Unknown"
                print(f"{Bolors.FAIL}Error Revert {Bolors.RESET}, oracle_post_result. Reason: Unknown")

            self._on_fail("oracle_post_result", message)

    def send_transaction(self, _round: int, dot: int, _hash: str, total_users: int, tx_params: Optional[TxParams] = None) -> Union[HexBytes, bytes]:
        """Execute underlying contract method via eth_sendTransaction.

        :param tx_params: transaction parameters
        """
        (_round, dot, _hash, total_users) = self.validate_and_normalize_inputs(_round, dot, _hash, total_users)
        tx_params = super().normalize_tx_params(tx_params)
        return self._underlying_method(_round, dot, _hash, total_users).transact(tx_params.as_dict())

    def build_transaction(self, _round: int, dot: int, _hash: str, total_users: int, tx_params: Optional[TxParams] = None) -> dict:
        """Construct calldata to be used as input to the method."""
        (_round, dot, _hash, total_users) = self.validate_and_normalize_inputs(_round, dot, _hash, total_users)
        tx_params = super().normalize_tx_params(tx_params)
        return self._underlying_method(_round, dot, _hash, total_users).buildTransaction(tx_params.as_dict())

    def estimate_gas(self, _round: int, dot: int, _hash: str, total_users: int, tx_params: Optional[TxParams] = None) -> int:
        """Estimate gas consumption of method call."""
        (_round, dot, _hash, total_users) = self.validate_and_normalize_inputs(_round, dot, _hash, total_users)
        tx_params = super().normalize_tx_params(tx_params)
        return self._underlying_method(_round, dot, _hash, total_users).estimateGas(tx_params.as_dict())
...
class PoolFeeMethod(ContractMethod): # pylint: disable=invalid-name
    """Various interfaces to the pool_fee method."""

    def __init__(self, elib: MiliDoS, contract_address: str, contract_function: ContractFunction, validator: Validator=None):
        """Persist instance data."""
        super().__init__(elib, contract_address, validator)
        self._underlying_method = contract_function
        self.sign = validator.getSignature("pool_fee")

    def validate_and_normalize_inputs(self, index_0: str)->any:
        """Validate the inputs to the pool_fee method."""
        self.validator.assert_valid(
            method_name='pool_fee',
            parameter_name='index_0',
            argument_value=index_0,
        )
        index_0 = self.validate_and_checksum_address(index_0)
    ...
    def block_send(self, index_0: str,_valeth:int=0) -> int:
        ...
        try:
            ...
            if 'data' in _t:
                ...
                self._on_receipt_handle("pool_fee", tx_receipt, txHash)
            ...
            print(f"{Bolors.FAIL}Error {er} {Bolors.RESET}: pool_fee")
            message = f"Error {er}: pool_fee"
            self._on_fail("pool_fee", message)
        except ValueError as err:
            if "message" in err.args[0]:
                message = err.args[0]["message"]
                print(f"{Bolors.FAIL}Error Revert {Bolors.RESET}, pool_fee: {message}")
            else:
                message = "Error Revert , Reason: Unknown"
                print(f"{Bolors.FAIL}Error Revert {Bolors.RESET}, pool_fee. Reason: Unknown")
...
class PoolFeeDiscountMethod(ContractMethod): # pylint: disable=invalid-name
    """Various interfaces to the pool_fee_discount method."""

    def __init__(self, elib: MiliDoS, contract_address: str, contract_function: ContractFunction, validator: Validator=None):
        """Persist instance data."""
        super().__init__(elib, contract_address, validator)
        self._underlying_method = contract_function
        self.sign = validator.getSignature("pool_fee_discount")

    def validate_and_normalize_inputs(self, index_0: str)->any:
        """Validate the inputs to the pool_fee_discount method."""
        self.validator.assert_valid(
            method_name='pool_fee_discount',
            parameter_name='index_0',
            argument_value=index_0,
        )
        index_0 = self.validate_and_checksum_address(index_0)
    ...
    def block_send(self, index_0: str,_valeth:int=0) -> int:
        ...
        try:
            ...
            if 'data' in _t:
                ...
                self._on_receipt_handle("pool_fee_discount", tx_receipt, txHash)
            ...
            print(f"{Bolors.FAIL}Error {er} {Bolors.RESET}: pool_fee_discount")
            message = f"Error {er}: pool_fee_discount"
            self._on_fail("pool_fee_discount", message)
        except ValueError as err:
            if "message" in err.args[0]:
                message = err.args[0]["message"]
                print(f"{Bolors.FAIL}Error Revert {Bolors.RESET}, pool_fee_discount: {message}")
            else:
                message = "Error Revert , Reason: Unknown"
                print(f"{Bolors.FAIL}Error Revert {Bolors.RESET}, pool_fee_discount. Reason: Unknown")
...
class PoolVaultMethod(ContractMethod): # pylint: disable=invalid-name
    """Various interfaces to the pool_vault method."""

    def __init__(self, elib: MiliDoS, contract_address: str, contract_function: ContractFunction, validator: Validator=None):
        """Persist instance data."""
        super().__init__(elib, contract_address, validator)
        self._underlying_method = contract_function
        self.sign = validator.getSignature("pool_vault")

    def validate_and_normalize_inputs(self, index_0: str)->any:
        """Validate the inputs to the pool_vault method."""
        self.validator.assert_valid(
            method_name='pool_vault',
            parameter_name='index_0',
            argument_value=index_0,
        )
        index_0 = self.validate_and_checksum_address(index_0)
    ...
    def block_send(self, index_0: str,_valeth:int=0) -> int:
        ...
        try:
            ...
            if 'data' in _t:
                ...
                self._on_receipt_handle("pool_vault", tx_receipt, txHash)
            ...
            print(f"{Bolors.FAIL}Error {er} {Bolors.RESET}: pool_vault")
            message = f"Error {er}: pool_vault"
            self._on_fail("pool_vault", message)
        except ValueError as err:
            if "message" in err.args[0]:
                message = err.args[0]["message"]
                print(f"{Bolors.FAIL}Error Revert {Bolors.RESET}, pool_vault: {message}")
            else:
                message = "Error Revert , Reason: Unknown"
                print(f"{Bolors.FAIL}Error Revert {Bolors.RESET}, pool_vault. Reason: Unknown")
...
class RoundVaultMethod(ContractMethod): # pylint: disable=invalid-name
    """Various interfaces to the round_vault method."""

    def __init__(self, elib: MiliDoS, contract_address: str, contract_function: ContractFunction, validator: Validator=None):
        """Persist instance data."""
        super().__init__(elib, contract_address, validator)
        self._underlying_method = contract_function
        self.sign = validator.getSignature("round_vault")

    def validate_and_normalize_inputs(self, index_0: int)->any:
        """Validate the inputs to the round_vault method."""
        self.validator.assert_valid(
            method_name='round_vault',
            parameter_name='index_0',
            argument_value=index_0,
        )
        # safeguard against fractional inputs
    ...
    def block_call(self,index_0: int, debug:bool=False) -> Tuple[int, int, str]:
        _fn = self._underlying_method(index_0)
        returned = _fn.call({
                'from': self._operate
            })
        return (returned[0],returned[1],returned[2],)
    def block_send(self, index_0: int,_valeth:int=0) -> Tuple[int, int, str]:
        """Execute underlying contract method via eth_call.
        ...
        try:
            ...
            if 'data' in _t:
                ...
                self._on_receipt_handle("round_vault", tx_receipt, txHash)
            ...
            print(f"{Bolors.FAIL}Error {er} {Bolors.RESET}: round_vault")
            message = f"Error {er}: round_vault"
            self._on_fail("round_vault", message)
        except ValueError as err:
            if "message" in err.args[0]:
                message = err.args[0]["message"]
                print(f"{Bolors.FAIL}Error Revert {Bolors.RESET}, round_vault: {message}")
            else:
                message = "Error Revert , Reason: Unknown"
                print(f"{Bolors.FAIL}Error Revert {Bolors.RESET}, round_vault. Reason: Unknown")
...
class SetDiscountMethod(ContractMethod): # pylint: disable=invalid-name
    """Various interfaces to the set_discount method."""

    def __init__(self, elib: MiliDoS, contract_address: str, contract_function: ContractFunction, validator: Validator=None):
        """Persist instance data."""
        super().__init__(elib, contract_address, validator)
        self._underlying_method = contract_function
        self.sign = validator.getSignature("set_discount")

    def validate_and_normalize_inputs(self, user: str, perc: int)->any:
        """Validate the inputs to the set_discount method."""
        self.validator.assert_valid(
            method_name='set_discount',
            parameter_name='user',
            argument_value=user,
        )
        user = self.validate_and_checksum_address(user)
    ...
    def block_send(self, user: str, perc: int,_valeth:int=0) -> None:
        """Execute underlying contract method via eth_call.

        :param tx_params: transaction parameters
        :returns: the return value of the underlying method.
        """
        _fn = self._underlying_method(user, perc)
        try:
            ...
            if 'data' in _t:
                ...
                self._on_receipt_handle("set_discount", tx_receipt, txHash)
            ...
            print(f"{Bolors.FAIL}Error {er} {Bolors.RESET}: set_discount")
            message = f"Error {er}: set_discount"
            self._on_fail("set_discount", message)
        except ValueError as err:
            if "message" in err.args[0]:
                message = err.args[0]["message"]
                print(f"{Bolors.FAIL}Error Revert {Bolors.RESET}, set_discount: {message}")
            else:
                message = "Error Revert , Reason: Unknown"
                print(f"{Bolors.FAIL}Error Revert {Bolors.RESET}, set_discount. Reason: Unknown")

            self._on_fail("set_discount", message)

    def send_transaction(self, user: str, perc: int, tx_params: Optional[TxParams] = None) -> Union[HexBytes, bytes]:
        """Execute underlying contract method via eth_sendTransaction.

        :param tx_params: transaction parameters
        """
        (user, perc) = self.validate_and_normalize_inputs(user, perc)
        tx_params = super().normalize_tx_params(tx_params)
        return self._underlying_method(user, perc).transact(tx_params.as_dict())

    def build_transaction(self, user: str, perc: int, tx_params: Optional[TxParams] = None) -> dict:
        """Construct calldata to be used as input to the method."""
        (user, perc) = self.validate_and_normalize_inputs(user, perc)
        tx_params = super().normalize_tx_params(tx_params)
        return self._underlying_method(user, perc).buildTransaction(tx_params.as_dict())

    def estimate_gas(self, user: str, perc: int, tx_params: Optional[TxParams] = None) -> int:
        """Estimate gas consumption of method call."""
        (user, perc) = self.validate_and_normalize_inputs(user, perc)
        tx_params = super().normalize_tx_params(tx_params)
        return self._underlying_method(user, perc).estimateGas(tx_params.as_dict())

class SetFeeMethod(ContractMethod): # pylint: disable=invalid-name
    """Various interfaces to the set_fee method."""

    def __init__(self, elib: MiliDoS, contract_address: str, contract_function: ContractFunction, validator: Validator=None):
        """Persist instance data."""
        super().__init__(elib, contract_address, validator)
        self._underlying_method = contract_function
        self.sign = validator.getSignature("set_fee")

    def validate_and_normalize_inputs(self, x: str, percent_1000: int)->any:
        """Validate the inputs to the set_fee method."""
        self.validator.assert_valid(
            method_name='set_fee',
            parameter_name='x',
            argument_value=x,
        ...
        self.validator.assert_valid(
            method_name='set_fee',
            parameter_name='percent_1000',
            argument_value=percent_1000,
        )
        return (x, percent_1000)



    def block_send(self, x: str, percent_1000: int,_valeth:int=0) -> None:
        """Execute underlying contract method via eth_call.

        :param tx_params: transaction parameters
        :returns: the return value of the underlying method.
        """
        _fn = self._underlying_method(x, percent_1000)
        try:
            ...
            if 'data' in _t:
                ...
                self._on_receipt_handle("set_fee", tx_receipt, txHash)
            ...
            print(f"{Bolors.FAIL}Error {er} {Bolors.RESET}: set_fee")
            message = f"Error {er}: set_fee"
            self._on_fail("set_fee", message)
        except ValueError as err:
            if "message" in err.args[0]:
                message = err.args[0]["message"]
                print(f"{Bolors.FAIL}Error Revert {Bolors.RESET}, set_fee: {message}")
            else:
                message = "Error Revert , Reason: Unknown"
                print(f"{Bolors.FAIL}Error Revert {Bolors.RESET}, set_fee. Reason: Unknown")

            self._on_fail("set_fee", message)

    def send_transaction(self, x: str, percent_1000: int, tx_params: Optional[TxParams] = None) -> Union[HexBytes, bytes]:
        """Execute underlying contract method via eth_sendTransaction.

        :param tx_params: transaction parameters
        """
        (x, percent_1000) = self.validate_and_normalize_inputs(x, percent_1000)
        tx_params = super().normalize_tx_params(tx_params)
        return self._underlying_method(x, percent_1000).transact(tx_params.as_dict())

    def build_transaction(self, x: str, percent_1000: int, tx_params: Optional[TxParams] = None) -> dict:
        """Construct calldata to be used as input to the method."""
        (x, percent_1000) = self.validate_and_normalize_inputs(x, percent_1000)
        tx_params = super().normalize_tx_params(tx_params)
        return self._underlying_method(x, percent_1000).buildTransaction(tx_params.as_dict())

    def estimate_gas(self, x: str, percent_1000: int, tx_params: Optional[TxParams] = None) -> int:
        """Estimate gas consumption of method call."""
        (x, percent_1000) = self.validate_and_normalize_inputs(x, percent_1000)
        tx_params = super().normalize_tx_params(tx_params)
        return self._underlying_method(x, percent_1000).estimateGas(tx_params.as_dict())

class SetRoundMethod(ContractMethod): # pylint: disable=invalid-name
    """Various interfaces to the set_round method."""

    def __init__(self, elib: MiliDoS, contract_address: str, contract_function: ContractFunction, validator: Validator=None):
        """Persist instance data."""
        super().__init__(elib, contract_address, validator)
        self._underlying_method = contract_function
        self.sign = validator.getSignature("set_round")

    def validate_and_normalize_inputs(self, n_round: int)->any:
        """Validate the inputs to the set_round method."""
        self.validator.assert_valid(
            method_name='set_round',
            parameter_name='n_round',
            argument_value=n_round,
        )
        # safeguard against fractional inputs
        n_round = int(n_round)
        return (n_round)
    ...
    def block_send(self, n_round: int,_valeth:int=0) -> None:
        """Execute underlying contract method via eth_call.

        :param tx_params: transaction parameters
        :returns: the return value of the underlying method.
        """
        _fn = self._underlying_method(n_round)
        try:
            ...
            if 'data' in _t:
                ...
                self._on_receipt_handle("set_round", tx_receipt, txHash)
            ...
            print(f"{Bolors.FAIL}Error {er} {Bolors.RESET}: set_round")
            message = f"Error {er}: set_round"
            self._on_fail("set_round", message)
        except ValueError as err:
            if "message" in err.args[0]:
                message = err.args[0]["message"]
                print(f"{Bolors.FAIL}Error Revert {Bolors.RESET}, set_round: {message}")
            else:
                message = "Error Revert , Reason: Unknown"
                print(f"{Bolors.FAIL}Error Revert {Bolors.RESET}, set_round. Reason: Unknown")

            self._on_fail("set_round", message)

    def send_transaction(self, n_round: int, tx_params: Optional[TxParams] = None) -> Union[HexBytes, bytes]:
        """Execute underlying contract method via eth_sendTransaction.

        :param tx_params: transaction parameters
        """
        (n_round) = self.validate_and_normalize_inputs(n_round)
        tx_params = super().normalize_tx_params(tx_params)
        return self._underlying_method(n_round).transact(tx_params.as_dict())

    def build_transaction(self, n_round: int, tx_params: Optional[TxParams] = None) -> dict:
        """Construct calldata to be used as input to the method."""
        (n_round) = self.validate_and_normalize_inputs(n_round)
        tx_params = super().normalize_tx_params(tx_params)
        return self._underlying_method(n_round).buildTransaction(tx_params.as_dict())

    def estimate_gas(self, n_round: int, tx_params: Optional[TxParams] = None) -> int:
        """Estimate gas consumption of method call."""
        (n_round) = self.validate_and_normalize_inputs(n_round)
        tx_params = super().normalize_tx_params(tx_params)
        return self._underlying_method(n_round).estimateGas(tx_params.as_dict())

class SignatureMethod(ContractMethod): # pylint: disable=invalid-name
    """Various interfaces to the signature method."""

    def __init__(self, elib: MiliDoS, contract_address: str, contract_function: ContractFunction, validator: Validator=None):
        """Persist instance data."""
        super().__init__(elib, contract_address, validator)
        self._underlying_method = contract_function
        self.sign = validator.getSignature("signature")

    def validate_and_normalize_inputs(self, user: str, coin: str)->any:
        """Validate the inputs to the signature method."""
        self.validator.assert_valid(
    ...
    def block_send(self, user: str, coin: str,_valeth:int=0) -> Union[bytes, str]:
        ...
        try:
            ...
            if 'data' in _t:
                ...
                self._on_receipt_handle("signature", tx_receipt, txHash)
            ...
            print(f"{Bolors.FAIL}Error {er} {Bolors.RESET}: signature")
            message = f"Error {er}: signature"
            self._on_fail("signature", message)
        except ValueError as err:
            if "message" in err.args[0]:
                message = err.args[0]["message"]
                print(f"{Bolors.FAIL}Error Revert {Bolors.RESET}, signature: {message}")
            else:
                message = "Error Revert , Reason: Unknown"
                print(f"{Bolors.FAIL}Error Revert {Bolors.RESET}, signature. Reason: Unknown")
...
class TakeUsrMethod(ContractMethod): # pylint: disable=invalid-name
    """Various interfaces to the take_usr method."""

    def __init__(self, elib: MiliDoS, contract_address: str, contract_function: ContractFunction, validator: Validator=None):
        """Persist instance data."""
        super().__init__(elib, contract_address, validator)
        self._underlying_method = contract_function
        self.sign = validator.getSignature("take_usr")

    def validate_and_normalize_inputs(self, user: str, coin: str, amount: int)->any:
        """Validate the inputs to the take_usr method."""
        self.validator.assert_valid(
            method_name='take_usr',
            parameter_name='user',
            argument_value=user,
        )
        user = self.validate_and_checksum_address(user)
    ...
    def block_send(self, user: str, coin: str, amount: int,_valeth:int=0) -> None:
        """Execute underlying contract method via eth_call.

        :param tx_params: transaction parameters
        :returns: the return value of the underlying method.
        """
        _fn = self._underlying_method(user, coin, amount)
        try:
            ...
            if 'data' in _t:
                ...
                self._on_receipt_handle("take_usr", tx_receipt, txHash)
            ...
            print(f"{Bolors.FAIL}Error {er} {Bolors.RESET}: take_usr")
            message = f"Error {er}: take_usr"
            self._on_fail("take_usr", message)
        except ValueError as err:
            if "message" in err.args[0]:
                message = err.args[0]["message"]
                print(f"{Bolors.FAIL}Error Revert {Bolors.RESET}, take_usr: {message}")
            else:
                message = "Error Revert , Reason: Unknown"
                print(f"{Bolors.FAIL}Error Revert {Bolors.RESET}, take_usr. Reason: Unknown")

            self._on_fail("take_usr", message)

    def send_transaction(self, user: str, coin: str, amount: int, tx_params: Optional[TxParams] = None) -> Union[HexBytes, bytes]:
        """Execute underlying contract method via eth_sendTransaction.

        :param tx_params: transaction parameters
        """
        (user, coin, amount) = self.validate_and_normalize_inputs(user, coin, amount)
        tx_params = super().normalize_tx_params(tx_params)
        return self._underlying_method(user, coin, amount).transact(tx_params.as_dict())

    def build_transaction(self, user: str, coin: str, amount: int, tx_params: Optional[TxParams] = None) -> dict:
        """Construct calldata to be used as input to the method."""
        (user, coin, amount) = self.validate_and_normalize_inputs(user, coin, amount)
        tx_params = super().normalize_tx_params(tx_params)
        return self._underlying_method(user, coin, amount).buildTransaction(tx_params.as_dict())

    def estimate_gas(self, user: str, coin: str, amount: int, tx_params: Optional[TxParams] = None) -> int:
        """Estimate gas consumption of method call."""
        (user, coin, amount) = self.validate_and_normalize_inputs(user, coin, amount)
        tx_params = super().normalize_tx_params(tx_params)
        return self._underlying_method(user, coin, amount).estimateGas(tx_params.as_dict())
...
class UpBankVaultMethod(ContractMethod): # pylint: disable=invalid-name
    """Various interfaces to the up_bank_vault method."""

    def __init__(self, elib: MiliDoS, contract_address: str, contract_function: ContractFunction, validator: Validator=None):
        """Persist instance data."""
        super().__init__(elib, contract_address, validator)
        self._underlying_method = contract_function
        self.sign = validator.getSignature("up_bank_vault")

    def validate_and_normalize_inputs(self, coins: List[str], balances: List[int])->any:
        """Validate the inputs to the up_bank_vault method."""
        self.validator.assert_valid(
            method_name='up_bank_vault',
            parameter_name='coins',
            argument_value=coins,
        )
        self.validator.assert_valid(
            method_name='up_bank_vault',
            parameter_name='balances',
            argument_value=balances,
        )
        return (coins, balances)



    def block_send(self, coins: List[str], balances: List[int],_valeth:int=0) -> None:
        """Execute underlying contract method via eth_call.

        :param tx_params: transaction parameters
        :returns: the return value of the underlying method.
        """
        _fn = self._underlying_method(coins, balances)
        try:
            ...
            if 'data' in _t:
                ...
                self._on_receipt_handle("up_bank_vault", tx_receipt, txHash)
            ...
            print(f"{Bolors.FAIL}Error {er} {Bolors.RESET}: up_bank_vault")
            message = f"Error {er}: up_bank_vault"
            self._on_fail("up_bank_vault", message)
        except ValueError as err:
            if "message" in err.args[0]:
                message = err.args[0]["message"]
                print(f"{Bolors.FAIL}Error Revert {Bolors.RESET}, up_bank_vault: {message}")
            else:
                message = "Error Revert , Reason: Unknown"
                print(f"{Bolors.FAIL}Error Revert {Bolors.RESET}, up_bank_vault. Reason: Unknown")

            self._on_fail("up_bank_vault", message)

    def send_transaction(self, coins: List[str], balances: List[int], tx_params: Optional[TxParams] = None) -> Union[HexBytes, bytes]:
        """Execute underlying contract method via eth_sendTransaction.

        :param tx_params: transaction parameters
        """
        (coins, balances) = self.validate_and_normalize_inputs(coins, balances)
        tx_params = super().normalize_tx_params(tx_params)
        return self._underlying_method(coins, balances).transact(tx_params.as_dict())

    def build_transaction(self, coins: List[str], balances: List[int], tx_params: Optional[TxParams] = None) -> dict:
        """Construct calldata to be used as input to the method."""
        (coins, balances) = self.validate_and_normalize_inputs(coins, balances)
        tx_params = super().normalize_tx_params(tx_params)
        return self._underlying_method(coins, balances).buildTransaction(tx_params.as_dict())

    def estimate_gas(self, coins: List[str], balances: List[int], tx_params: Optional[TxParams] = None) -> int:
        """Estimate gas consumption of method call."""
        (coins, balances) = self.validate_and_normalize_inputs(coins, balances)
        tx_params = super().normalize_tx_params(tx_params)
        return self._underlying_method(coins, balances).estimateGas(tx_params.as_dict())

class UpSingleMethod(ContractMethod): # pylint: disable=invalid-name
    """Various interfaces to the up_single method."""

    def __init__(self, elib: MiliDoS, contract_address: str, contract_function: ContractFunction, validator: Validator=None):
        """Persist instance data."""
        super().__init__(elib, contract_address, validator)
        self._underlying_method = contract_function
        self.sign = validator.getSignature("up_single")

    def validate_and_normalize_inputs(self, user: str, coin: str, amount: int)->any:
        """Validate the inputs to the up_single method."""
        self.validator.assert_valid(
            method_name='up_single',
            parameter_name='user',
            argument_value=user,
        )
        user = self.validate_and_checksum_address(user)
    ...
    def block_send(self, user: str, coin: str, amount: int,_valeth:int=0) -> None:
        ...
        try:
            ...
            if 'data' in _t:
                ...
                self._on_receipt_handle("up_single", tx_receipt, txHash)
            ...
            print(f"{Bolors.FAIL}Error {er} {Bolors.RESET}: up_single")
            message = f"Error {er}: up_single"
            self._on_fail("up_single", message)
        except ValueError as err:
            if "message" in err.args[0]:
                message = err.args[0]["message"]
                print(f"{Bolors.FAIL}Error Revert {Bolors.RESET}, up_single: {message}")
            else:
                message = "Error Revert , Reason: Unknown"
                print(f"{Bolors.FAIL}Error Revert {Bolors.RESET}, up_single. Reason: Unknown")
...
class UpUsrBatchMethod(ContractMethod): # pylint: disable=invalid-name
    """Various interfaces to the up_usr_batch method."""

    def __init__(self, elib: MiliDoS, contract_address: str, contract_function: ContractFunction, validator: Validator=None):
        """Persist instance data."""
        super().__init__(elib, contract_address, validator)
        self._underlying_method = contract_function
        self.sign = validator.getSignature("up_usr_batch")

    def validate_and_normalize_inputs(self, users: List[str], coins: List[str], amounts: List[int])->any:
        """Validate the inputs to the up_usr_batch method."""
        self.validator.assert_valid(
            method_name='up_usr_batch',
            parameter_name='users',
            argument_value=users,
        )
        self.validator.assert_valid(
            method_name='up_usr_batch',
            parameter_name='coins',
        ...
        return (users, coins, amounts)



    def block_send(self, users: List[str], coins: List[str], amounts: List[int],_valeth:int=0) -> None:
        """Execute underlying contract method via eth_call.

        :param tx_params: transaction parameters
        :returns: the return value of the underlying method.
        """
        _fn = self._underlying_method(users, coins, amounts)
        try:
            ...
            if 'data' in _t:
                ...
                self._on_receipt_handle("up_usr_batch", tx_receipt, txHash)
            ...
            print(f"{Bolors.FAIL}Error {er} {Bolors.RESET}: up_usr_batch")
            message = f"Error {er}: up_usr_batch"
            self._on_fail("up_usr_batch", message)
        except ValueError as err:
            if "message" in err.args[0]:
                message = err.args[0]["message"]
                print(f"{Bolors.FAIL}Error Revert {Bolors.RESET}, up_usr_batch: {message}")
            else:
                message = "Error Revert , Reason: Unknown"
                print(f"{Bolors.FAIL}Error Revert {Bolors.RESET}, up_usr_batch. Reason: Unknown")

            self._on_fail("up_usr_batch", message)

    def send_transaction(self, users: List[str], coins: List[str], amounts: List[int], tx_params: Optional[TxParams] = None) -> Union[HexBytes, bytes]:
        """Execute underlying contract method via eth_sendTransaction.

        :param tx_params: transaction parameters
        """
        (users, coins, amounts) = self.validate_and_normalize_inputs(users, coins, amounts)
        tx_params = super().normalize_tx_params(tx_params)
        return self._underlying_method(users, coins, amounts).transact(tx_params.as_dict())

    def build_transaction(self, users: List[str], coins: List[str], amounts: List[int], tx_params: Optional[TxParams] = None) -> dict:
        """Construct calldata to be used as input to the method."""
        (users, coins, amounts) = self.validate_and_normalize_inputs(users, coins, amounts)
        tx_params = super().normalize_tx_params(tx_params)
        return self._underlying_method(users, coins, amounts).buildTransaction(tx_params.as_dict())

    def estimate_gas(self, users: List[str], coins: List[str], amounts: List[int], tx_params: Optional[TxParams] = None) -> int:
        """Estimate gas consumption of method call."""
        (users, coins, amounts) = self.validate_and_normalize_inputs(users, coins, amounts)
        tx_params = super().normalize_tx_params(tx_params)
        return self._underlying_method(users, coins, amounts).estimateGas(tx_params.as_dict())

class UpdateProviderMethod(ContractMethod): # pylint: disable=invalid-name
    """Various interfaces to the updateProvider method."""

    def __init__(self, elib: MiliDoS, contract_address: str, contract_function: ContractFunction, validator: Validator=None):
        """Persist instance data."""
        super().__init__(elib, contract_address, validator)
        self._underlying_method = contract_function
        self.sign = validator.getSignature("updateProvider")

    def validate_and_normalize_inputs(self, k: str)->any:
        """Validate the inputs to the updateProvider method."""
        self.validator.assert_valid(
            method_name='updateProvider',
            parameter_name='k',
            argument_value=k,
        )
        k = self.validate_and_checksum_address(k)
        return (k)



    def block_send(self, k: str,_valeth:int=0) -> None:
        """Execute underlying contract method via eth_call.

        :param tx_params: transaction parameters
        :returns: the return value of the underlying method.
        """
        _fn = self._underlying_method(k)
        try:
            ...
            if 'data' in _t:
                ...
                self._on_receipt_handle("update_provider", tx_receipt, txHash)
            ...
            print(f"{Bolors.FAIL}Error {er} {Bolors.RESET}: update_provider")
            message = f"Error {er}: update_provider"
            self._on_fail("update_provider", message)
        except ValueError as err:
            if "message" in err.args[0]:
                message = err.args[0]["message"]
                print(f"{Bolors.FAIL}Error Revert {Bolors.RESET}, update_provider: {message}")
            else:
                message = "Error Revert , Reason: Unknown"
                print(f"{Bolors.FAIL}Error Revert {Bolors.RESET}, update_provider. Reason: Unknown")

            self._on_fail("update_provider", message)

    def send_transaction(self, k: str, tx_params: Optional[TxParams] = None) -> Union[HexBytes, bytes]:
        """Execute underlying contract method via eth_sendTransaction.
    ...
    def build_transaction(self, k: str, tx_params: Optional[TxParams] = None) -> dict:
        """Construct calldata to be used as input to the method."""
        (k) = self.validate_and_normalize_inputs(k)
        tx_params = super().normalize_tx_params(tx_params)
        return self._underlying_method(k).buildTransaction(tx_params.as_dict())

    def estimate_gas(self, k: str, tx_params: Optional[TxParams] = None) -> int:
        """Estimate gas consumption of method call."""
        (k) = self.validate_and_normalize_inputs(k)
        tx_params = super().normalize_tx_params(tx_params)
...
class SignatureGenerator(Signatures):
    ...
    def bank_vault(self) -> str:
        return self._function_signatures["bank_vault"]
    def check(self) -> str:
        return self._function_signatures["check"]
    def deposit_erc20(self) -> str:
        return self._function_signatures["deposit_erc20"]
    def donate_erc20(self) -> str:
        return self._function_signatures["donate_erc20"]
    def game_round(self) -> str:
        return self._function_signatures["game_round"]
    ...
    def oracle_post_result(self) -> str:
        return self._function_signatures["oracle_post_result"]
    def owner(self) -> str:
        return self._function_signatures["owner"]
    def pause_sc(self) -> str:
        return self._function_signatures["pauseSc"]
    def pool_fee(self) -> str:
        return self._function_signatures["pool_fee"]
    def pool_fee_discount(self) -> str:
        return self._function_signatures["pool_fee_discount"]
    def pool_vault(self) -> str:
        return self._function_signatures["pool_vault"]
    ...
    def round_vault(self) -> str:
        return self._function_signatures["round_vault"]
    def set_discount(self) -> str:
        return self._function_signatures["set_discount"]
    def set_fee(self) -> str:
        return self._function_signatures["set_fee"]
    def set_round(self) -> str:
        return self._function_signatures["set_round"]
    def signature(self) -> str:
        return self._function_signatures["signature"]
    def take_usr(self) -> str:
        return self._function_signatures["take_usr"]
    ...
    def up_bank_vault(self) -> str:
        return self._function_signatures["up_bank_vault"]
    def up_single(self) -> str:
        return self._function_signatures["up_single"]
    def up_usr_batch(self) -> str:
        return self._function_signatures["up_usr_batch"]
    def update_provider(self) -> str:
        return self._function_signatures["updateProvider"]
    def whitelist_admins(self) -> str:
        return self._function_signatures["whitelistAdmins"]

# pylint: disable=too-many-public-methods,too-many-instance-attributes
class Enrockvault(ContractBase):
    """Wrapper class for Enrockvault Solidity contract."""
    ...
    _fn_bank_vault: BankVaultMethod
    """Constructor-initialized instance of
    :class:`BankVaultMethod`.
    """

    _fn_check: CheckMethod
    """Constructor-initialized instance of
    :class:`CheckMethod`.
    """
    ...
    _fn_donate_erc20: DonateErc20Method
    """Constructor-initialized instance of
    :class:`DonateErc20Method`.
    """

    _fn_game_round: GameRoundMethod
    """Constructor-initialized instance of
    :class:`GameRoundMethod`.
    """
    ...
    _fn_oracle_post_result: OraclePostResultMethod
    """Constructor-initialized instance of
    :class:`OraclePostResultMethod`.
    """
    ...
    _fn_pool_fee: PoolFeeMethod
    """Constructor-initialized instance of
    :class:`PoolFeeMethod`.
    """

    _fn_pool_fee_discount: PoolFeeDiscountMethod
    """Constructor-initialized instance of
    :class:`PoolFeeDiscountMethod`.
    """

    _fn_pool_vault: PoolVaultMethod
    """Constructor-initialized instance of
    :class:`PoolVaultMethod`.
    """
    ...
    _fn_round_vault: RoundVaultMethod
    """Constructor-initialized instance of
    :class:`RoundVaultMethod`.
    """

    _fn_set_discount: SetDiscountMethod
    """Constructor-initialized instance of
    :class:`SetDiscountMethod`.
    """

    _fn_set_fee: SetFeeMethod
    """Constructor-initialized instance of
    :class:`SetFeeMethod`.
    """

    _fn_set_round: SetRoundMethod
    """Constructor-initialized instance of
    :class:`SetRoundMethod`.
    """

    _fn_signature: SignatureMethod
    """Constructor-initialized instance of
    :class:`SignatureMethod`.
    """

    _fn_take_usr: TakeUsrMethod
    """Constructor-initialized instance of
    :class:`TakeUsrMethod`.
    """
    ...
    _fn_up_bank_vault: UpBankVaultMethod
    """Constructor-initialized instance of
    :class:`UpBankVaultMethod`.
    """

    _fn_up_single: UpSingleMethod
    """Constructor-initialized instance of
    :class:`UpSingleMethod`.
    """

    _fn_up_usr_batch: UpUsrBatchMethod
    """Constructor-initialized instance of
    :class:`UpUsrBatchMethod`.
    """

    _fn_update_provider: UpdateProviderMethod
    """Constructor-initialized instance of
    :class:`UpdateProviderMethod`.
    """
    ...
    def __init__(
        ...
        validator: EnrockvaultValidator = None,
    ):
        """Get an instance of wrapper for smart contract.
        """
        # pylint: disable=too-many-statements
        super().__init__(contract_address, Enrockvault.abi())
        web3 = core_lib.w3

        if not validator:
            validator = EnrockvaultValidator(web3, contract_address)
        ...
        functions = self._web3_eth.contract(address=to_checksum_address(contract_address), abi=Enrockvault.abi()).functions
        self._signatures = SignatureGenerator(Enrockvault.abi())
        ...
        self._fn_bank_vault = BankVaultMethod(core_lib, contract_address, functions.bank_vault, validator)
        self._fn_check = CheckMethod(core_lib, contract_address, functions.check, validator)
        self._fn_deposit_erc20 = DepositErc20Method(core_lib, contract_address, functions.deposit_erc20, validator)
        self._fn_donate_erc20 = DonateErc20Method(core_lib, contract_address, functions.donate_erc20, validator)
        self._fn_game_round = GameRoundMethod(core_lib, contract_address, functions.game_round, validator)
        ...
        self._fn_oracle_post_result = OraclePostResultMethod(core_lib, contract_address, functions.oracle_post_result, validator)
        ...
        self._fn_pool_fee = PoolFeeMethod(core_lib, contract_address, functions.pool_fee, validator)
        self._fn_pool_fee_discount = PoolFeeDiscountMethod(core_lib, contract_address, functions.pool_fee_discount, validator)
        self._fn_pool_vault = PoolVaultMethod(core_lib, contract_address, functions.pool_vault, validator)
        ...
        self._fn_round_vault = RoundVaultMethod(core_lib, contract_address, functions.round_vault, validator)
        self._fn_set_discount = SetDiscountMethod(core_lib, contract_address, functions.set_discount, validator)
        self._fn_set_fee = SetFeeMethod(core_lib, contract_address, functions.set_fee, validator)
        self._fn_set_round = SetRoundMethod(core_lib, contract_address, functions.set_round, validator)
        self._fn_signature = SignatureMethod(core_lib, contract_address, functions.signature, validator)
        self._fn_take_usr = TakeUsrMethod(core_lib, contract_address, functions.take_usr, validator)
        ...
        self._fn_up_bank_vault = UpBankVaultMethod(core_lib, contract_address, functions.up_bank_vault, validator)
        self._fn_up_single = UpSingleMethod(core_lib, contract_address, functions.up_single, validator)
        self._fn_up_usr_batch = UpUsrBatchMethod(core_lib, contract_address, functions.up_usr_batch, validator)
        self._fn_update_provider = UpdateProviderMethod(core_lib, contract_address, functions.updateProvider, validator)
        self._fn_whitelist_admins = WhitelistAdminsMethod(core_lib, contract_address, functions.whitelistAdmins, validator)

    
    
    def event_donate_deposit(
            self, tx_hash: Union[HexBytes, bytes]
    ) -> Tuple[AttributeDict]:
        """
        Implementation of event donate_deposit in contract Enrockvault
        Get log entry for DonateDeposit event.
                :param tx_hash: hash of transaction emitting DonateDeposit event
        """
        tx_receipt = self._web3_eth.getTransactionReceipt(tx_hash)
        return self._web3_eth.contract(address=to_checksum_address(self.contract_address), abi=Enrockvault.abi()).events.DonateDeposit().processReceipt(tx_receipt)
    
    
    def event_game_vault(
            self, tx_hash: Union[HexBytes, bytes]
    ) -> Tuple[AttributeDict]:
        """
        Implementation of event game_vault in contract Enrockvault
        Get log entry for GameVault event.
                :param tx_hash: hash of transaction emitting GameVault event
        """
        tx_receipt = self._web3_eth.getTransactionReceipt(tx_hash)
        return self._web3_eth.contract(address=to_checksum_address(self.contract_address), abi=Enrockvault.abi()).events.GameVault().processReceipt(tx_receipt)
    ...
    def event_ownership_transferred(
        ...
        """
        Implementation of event ownership_transferred in contract Enrockvault
        Get log entry for OwnershipTransferred event.
                :param tx_hash: hash of transaction emitting OwnershipTransferred event
        """
        tx_receipt = self._web3_eth.getTransactionReceipt(tx_hash)
        return self._web3_eth.contract(address=to_checksum_address(self.contract_address), abi=Enrockvault.abi()).events.OwnershipTransferred().processReceipt(tx_receipt)
    ...
    def event_role_granted(
        ...
        """
        Implementation of event role_granted in contract Enrockvault
        Get log entry for RoleGranted event.
                :param tx_hash: hash of transaction emitting RoleGranted event
        """
        tx_receipt = self._web3_eth.getTransactionReceipt(tx_hash)
        return self._web3_eth.contract(address=to_checksum_address(self.contract_address), abi=Enrockvault.abi()).events.RoleGranted().processReceipt(tx_receipt)
    ...
    def event_role_revoked(
        ...
        """
        Implementation of event role_revoked in contract Enrockvault
        Get log entry for RoleRevoked event.
                :param tx_hash: hash of transaction emitting RoleRevoked event
        """
        tx_receipt = self._web3_eth.getTransactionReceipt(tx_hash)
        return self._web3_eth.contract(address=to_checksum_address(self.contract_address), abi=Enrockvault.abi()).events.RoleRevoked().processReceipt(tx_receipt)
    
    
    def event_updated_bank(
            self, tx_hash: Union[HexBytes, bytes]
    ) -> Tuple[AttributeDict]:
        """
        Implementation of event updated_bank in contract Enrockvault
        Get log entry for UpdatedBank event.
                :param tx_hash: hash of transaction emitting UpdatedBank event
        """
        tx_receipt = self._web3_eth.getTransactionReceipt(tx_hash)
        return self._web3_eth.contract(address=to_checksum_address(self.contract_address), abi=Enrockvault.abi()).events.UpdatedBank().processReceipt(tx_receipt)
    
    
    def event_updated_wallet(
            self, tx_hash: Union[HexBytes, bytes]
    ) -> Tuple[AttributeDict]:
        """
        Implementation of event updated_wallet in contract Enrockvault
        Get log entry for UpdatedWallet event.
                :param tx_hash: hash of transaction emitting UpdatedWallet event
        """
        tx_receipt = self._web3_eth.getTransactionReceipt(tx_hash)
        return self._web3_eth.contract(address=to_checksum_address(self.contract_address), abi=Enrockvault.abi()).events.UpdatedWallet().processReceipt(tx_receipt)
    ...
    def event_usr_deposit(
        ...
        """
        Implementation of event usr_deposit in contract Enrockvault
        Get log entry for UsrDeposit event.
                :param tx_hash: hash of transaction emitting UsrDeposit event
        """
        tx_receipt = self._web3_eth.getTransactionReceipt(tx_hash)
        return self._web3_eth.contract(address=to_checksum_address(self.contract_address), abi=Enrockvault.abi()).events.UsrDeposit().processReceipt(tx_receipt)
    
    
    def event_usr_withdraw(
            self, tx_hash: Union[HexBytes, bytes]
    ) -> Tuple[AttributeDict]:
        """
        Implementation of event usr_withdraw in contract Enrockvault
        Get log entry for UsrWithdraw event.
                :param tx_hash: hash of transaction emitting UsrWithdraw event
        """
        tx_receipt = self._web3_eth.getTransactionReceipt(tx_hash)
        return self._web3_eth.contract(address=to_checksum_address(self.contract_address), abi=Enrockvault.abi()).events.UsrWithdraw().processReceipt(tx_receipt)
    
    
    def event_withdrawal_gov(
            self, tx_hash: Union[HexBytes, bytes]
    ) -> Tuple[AttributeDict]:
        """
        Implementation of event withdrawal_gov in contract Enrockvault
        Get log entry for WithdrawalGov event.
                :param tx_hash: hash of transaction emitting WithdrawalGov event
        """
        tx_receipt = self._web3_eth.getTransactionReceipt(tx_hash)
        return self._web3_eth.contract(address=to_checksum_address(self.contract_address), abi=Enrockvault.abi()).events.WithdrawalGov().processReceipt(tx_receipt)
    ...
    def event_contract_paused(
        ...
        """
        Implementation of event contract_paused in contract Enrockvault
        Get log entry for contractPaused event.
                :param tx_hash: hash of transaction emitting contractPaused event
        """
        tx_receipt = self._web3_eth.getTransactionReceipt(tx_hash)
        return self._web3_eth.contract(address=to_checksum_address(self.contract_address), abi=Enrockvault.abi()).events.contractPaused().processReceipt(tx_receipt)
    ...
    def event_traillock(
        ...
        """
        Implementation of event traillock in contract Enrockvault
        Get log entry for traillock event.
                :param tx_hash: hash of transaction emitting traillock event
        """
        tx_receipt = self._web3_eth.getTransactionReceipt(tx_hash)
        return self._web3_eth.contract(address=to_checksum_address(self.contract_address), abi=Enrockvault.abi()).events.traillock().processReceipt(tx_receipt)
    ...
    def default_admin_role(self) -> Union[bytes, str]:
        """
        Implementation of default_admin_role in contract Enrockvault
    ...
    def add_governor(self, account: str) -> None:
        """
        Implementation of add_governor in contract Enrockvault
        Method of the function
    ...
    def add_whitelist_admin(self, account: str) -> None:
        """
        Implementation of add_whitelist_admin in contract Enrockvault
        Method of the function
    ...
    def bank_vault(self, index_0: str) -> int:
        """
        Implementation of bank_vault in contract Enrockvault
        Method of the function
    
        """
    
        self._fn_bank_vault.callback_onfail = self._callback_onfail
        self._fn_bank_vault.callback_onsuccess = self._callback_onsuccess
        self._fn_bank_vault.auto_reciept = self.call_contract_enforce_tx_receipt
        self._fn_bank_vault.gas_limit = self.call_contract_fee_amount
        self._fn_bank_vault.gas_price_wei = self.call_contract_fee_price
        self._fn_bank_vault.debug_method = self.call_contract_debug_flag
        ...
        return self._fn_bank_vault.block_call(index_0)
    
    
    
    def check(self, coin: str, user: str) -> int:
        """
        Implementation of check in contract Enrockvault
        Method of the function
    
        """
    
        self._fn_check.callback_onfail = self._callback_onfail
        self._fn_check.callback_onsuccess = self._callback_onsuccess
        self._fn_check.auto_reciept = self.call_contract_enforce_tx_receipt
        self._fn_check.gas_limit = self.call_contract_fee_amount
        self._fn_check.gas_price_wei = self.call_contract_fee_price
        self._fn_check.debug_method = self.call_contract_debug_flag
        ...
        return self._fn_check.block_call(coin, user)
    
    
    
    def deposit_erc20(self, coin: str, amount: int) -> None:
        """
        Implementation of deposit_erc20 in contract Enrockvault
        Method of the function
    ...
    def donate_erc20(self, coin: str, amount: int) -> None:
        """
        Implementation of donate_erc20 in contract Enrockvault
        Method of the function
    
        """
    
        self._fn_donate_erc20.callback_onfail = self._callback_onfail
        self._fn_donate_erc20.callback_onsuccess = self._callback_onsuccess
        self._fn_donate_erc20.auto_reciept = self.call_contract_enforce_tx_receipt
        self._fn_donate_erc20.gas_limit = self.call_contract_fee_amount
        self._fn_donate_erc20.gas_price_wei = self.call_contract_fee_price
        self._fn_donate_erc20.debug_method = self.call_contract_debug_flag
    
    
        return self._fn_donate_erc20.block_send(coin, amount)
    ...
    def game_round(self) -> int:
        """
        Implementation of game_round in contract Enrockvault
        Method of the function
    
        """
    
        self._fn_game_round.callback_onfail = self._callback_onfail
        self._fn_game_round.callback_onsuccess = self._callback_onsuccess
        self._fn_game_round.auto_reciept = self.call_contract_enforce_tx_receipt
        self._fn_game_round.gas_limit = self.call_contract_fee_amount
        self._fn_game_round.gas_price_wei = self.call_contract_fee_price
        self._fn_game_round.debug_method = self.call_contract_debug_flag
        ...
        return self._fn_game_round.block_call()
    
    
    
    def get_role_admin(self, role: Union[bytes, str]) -> Union[bytes, str]:
        """
        Implementation of get_role_admin in contract Enrockvault
        Method of the function
    ...
    def get_role_member(self, role: Union[bytes, str], index: int) -> str:
        """
        Implementation of get_role_member in contract Enrockvault
        Method of the function
    ...
    def get_role_member_count(self, role: Union[bytes, str]) -> int:
        """
        Implementation of get_role_member_count in contract Enrockvault
        Method of the function
    ...
    def governor(self) -> Union[bytes, str]:
        """
        Implementation of governor in contract Enrockvault
        Method of the function
    ...
    def grant_role(self, role: Union[bytes, str], account: str) -> None:
        """
        Implementation of grant_role in contract Enrockvault
        Method of the function
    ...
    def has_role(self, role: Union[bytes, str], account: str) -> bool:
        """
        Implementation of has_role in contract Enrockvault
        Method of the function
    ...
    def is_governor(self, account: str) -> bool:
        """
        Implementation of is_governor in contract Enrockvault
        Method of the function
    ...
    def is_locked(self) -> bool:
        """
        Implementation of is_locked in contract Enrockvault
        Method of the function
    ...
    def is_owner(self) -> bool:
        """
        Implementation of is_owner in contract Enrockvault
        Method of the function
    ...
    def is_paused(self) -> bool:
        """
        Implementation of is_paused in contract Enrockvault
        Method of the function
    ...
    def is_whitelist_admin(self, account: str) -> bool:
        """
        Implementation of is_whitelist_admin in contract Enrockvault
        Method of the function
    ...
    def lock(self) -> None:
        """
        Implementation of lock in contract Enrockvault
        Method of the function
    ...
    def oracle_post_result(self, _round: int, dot: int, _hash: str, total_users: int) -> None:
        """
        Implementation of oracle_post_result in contract Enrockvault
        Method of the function
    
        """
    
        self._fn_oracle_post_result.callback_onfail = self._callback_onfail
        self._fn_oracle_post_result.callback_onsuccess = self._callback_onsuccess
        self._fn_oracle_post_result.auto_reciept = self.call_contract_enforce_tx_receipt
        self._fn_oracle_post_result.gas_limit = self.call_contract_fee_amount
        self._fn_oracle_post_result.gas_price_wei = self.call_contract_fee_price
        self._fn_oracle_post_result.debug_method = self.call_contract_debug_flag
    
    
        return self._fn_oracle_post_result.block_send(_round, dot, _hash, total_users)
    ...
    def owner(self) -> str:
        """
        Implementation of owner in contract Enrockvault
        Method of the function
    ...
    def pause_sc(self) -> None:
        """
        Implementation of pause_sc in contract Enrockvault
        Method of the function
    ...
    def pool_fee(self, index_0: str) -> int:
        """
        Implementation of pool_fee in contract Enrockvault
        Method of the function
    
        """
    
        self._fn_pool_fee.callback_onfail = self._callback_onfail
        self._fn_pool_fee.callback_onsuccess = self._callback_onsuccess
        self._fn_pool_fee.auto_reciept = self.call_contract_enforce_tx_receipt
        self._fn_pool_fee.gas_limit = self.call_contract_fee_amount
        self._fn_pool_fee.gas_price_wei = self.call_contract_fee_price
        self._fn_pool_fee.debug_method = self.call_contract_debug_flag
        ...
        return self._fn_pool_fee.block_call(index_0)
    
    
    
    def pool_fee_discount(self, index_0: str) -> int:
        """
        Implementation of pool_fee_discount in contract Enrockvault
        Method of the function
    
        """
    
        self._fn_pool_fee_discount.callback_onfail = self._callback_onfail
        self._fn_pool_fee_discount.callback_onsuccess = self._callback_onsuccess
        self._fn_pool_fee_discount.auto_reciept = self.call_contract_enforce_tx_receipt
        self._fn_pool_fee_discount.gas_limit = self.call_contract_fee_amount
        self._fn_pool_fee_discount.gas_price_wei = self.call_contract_fee_price
        self._fn_pool_fee_discount.debug_method = self.call_contract_debug_flag
        ...
        return self._fn_pool_fee_discount.block_call(index_0)
    
    
    
    def pool_vault(self, index_0: str) -> int:
        """
        Implementation of pool_vault in contract Enrockvault
        Method of the function
    
        """
    
        self._fn_pool_vault.callback_onfail = self._callback_onfail
        self._fn_pool_vault.callback_onsuccess = self._callback_onsuccess
        self._fn_pool_vault.auto_reciept = self.call_contract_enforce_tx_receipt
        self._fn_pool_vault.gas_limit = self.call_contract_fee_amount
        self._fn_pool_vault.gas_price_wei = self.call_contract_fee_price
        self._fn_pool_vault.debug_method = self.call_contract_debug_flag
        ...
        return self._fn_pool_vault.block_call(index_0)
    
    
    
    def remove_governor(self, account: str) -> None:
        """
        Implementation of remove_governor in contract Enrockvault
        Method of the function
    ...
    def remove_whitelist_admin(self, account: str) -> None:
        """
        Implementation of remove_whitelist_admin in contract Enrockvault
        Method of the function
    ...
    def renounce_ownership(self) -> None:
        """
        Implementation of renounce_ownership in contract Enrockvault
        Method of the function
    ...
    def renounce_role(self, role: Union[bytes, str], account: str) -> None:
        """
        Implementation of renounce_role in contract Enrockvault
        Method of the function
    ...
    def revoke_role(self, role: Union[bytes, str], account: str) -> None:
        """
        Implementation of revoke_role in contract Enrockvault
        Method of the function
    ...
    def round_vault(self, index_0: int) -> Tuple[int, int, str]:
        """
        Implementation of round_vault in contract Enrockvault
        Method of the function
    
        """
    
        self._fn_round_vault.callback_onfail = self._callback_onfail
        self._fn_round_vault.callback_onsuccess = self._callback_onsuccess
        self._fn_round_vault.auto_reciept = self.call_contract_enforce_tx_receipt
        self._fn_round_vault.gas_limit = self.call_contract_fee_amount
        self._fn_round_vault.gas_price_wei = self.call_contract_fee_price
        self._fn_round_vault.debug_method = self.call_contract_debug_flag
        ...
        return self._fn_round_vault.block_call(index_0)
    
    
    
    def set_discount(self, user: str, perc: int) -> None:
        """
        Implementation of set_discount in contract Enrockvault
        Method of the function
    
        """
    
        self._fn_set_discount.callback_onfail = self._callback_onfail
        self._fn_set_discount.callback_onsuccess = self._callback_onsuccess
        self._fn_set_discount.auto_reciept = self.call_contract_enforce_tx_receipt
        self._fn_set_discount.gas_limit = self.call_contract_fee_amount
        self._fn_set_discount.gas_price_wei = self.call_contract_fee_price
        self._fn_set_discount.debug_method = self.call_contract_debug_flag
    
    
        return self._fn_set_discount.block_send(user, perc)
    ...
    def set_fee(self, x: str, percent_1000: int) -> None:
        """
        Implementation of set_fee in contract Enrockvault
        Method of the function
    
        """
    
        self._fn_set_fee.callback_onfail = self._callback_onfail
        self._fn_set_fee.callback_onsuccess = self._callback_onsuccess
        self._fn_set_fee.auto_reciept = self.call_contract_enforce_tx_receipt
        self._fn_set_fee.gas_limit = self.call_contract_fee_amount
        self._fn_set_fee.gas_price_wei = self.call_contract_fee_price
        self._fn_set_fee.debug_method = self.call_contract_debug_flag
    
    
        return self._fn_set_fee.block_send(x, percent_1000)
    ...
    def set_round(self, n_round: int) -> None:
        """
        Implementation of set_round in contract Enrockvault
        Method of the function
    
        """
    
        self._fn_set_round.callback_onfail = self._callback_onfail
        self._fn_set_round.callback_onsuccess = self._callback_onsuccess
        self._fn_set_round.auto_reciept = self.call_contract_enforce_tx_receipt
        self._fn_set_round.gas_limit = self.call_contract_fee_amount
        self._fn_set_round.gas_price_wei = self.call_contract_fee_price
        self._fn_set_round.debug_method = self.call_contract_debug_flag
    
    
        return self._fn_set_round.block_send(n_round)
    ...
    def signature(self, user: str, coin: str) -> Union[bytes, str]:
        """
        Implementation of signature in contract Enrockvault
        Method of the function
    
        """
    
        self._fn_signature.callback_onfail = self._callback_onfail
        self._fn_signature.callback_onsuccess = self._callback_onsuccess
        self._fn_signature.auto_reciept = self.call_contract_enforce_tx_receipt
        self._fn_signature.gas_limit = self.call_contract_fee_amount
        self._fn_signature.gas_price_wei = self.call_contract_fee_price
        self._fn_signature.debug_method = self.call_contract_debug_flag
    
    
    
    
        return self._fn_signature.block_call(user, coin)
    
    
    
    
    
    def take_usr(self, user: str, coin: str, amount: int) -> None:
        """
        Implementation of take_usr in contract Enrockvault
        Method of the function
    
        """
    
        self._fn_take_usr.callback_onfail = self._callback_onfail
        self._fn_take_usr.callback_onsuccess = self._callback_onsuccess
        self._fn_take_usr.auto_reciept = self.call_contract_enforce_tx_receipt
        self._fn_take_usr.gas_limit = self.call_contract_fee_amount
        self._fn_take_usr.gas_price_wei = self.call_contract_fee_price
        self._fn_take_usr.debug_method = self.call_contract_debug_flag
    
    
        return self._fn_take_usr.block_send(user, coin, amount)
    ...
    def transfer_ownership(self, new_owner: str) -> None:
        """
        Implementation of transfer_ownership in contract Enrockvault
        Method of the function
    ...
    def unlock(self) -> None:
        """
        Implementation of unlock in contract Enrockvault
        Method of the function
    ...
    def unpause_sc(self) -> None:
        """
        Implementation of unpause_sc in contract Enrockvault
        Method of the function
    ...
    def up_bank_vault(self, coins: List[str], balances: List[int]) -> None:
        """
        Implementation of up_bank_vault in contract Enrockvault
        Method of the function
    
        """
    
        self._fn_up_bank_vault.callback_onfail = self._callback_onfail
        self._fn_up_bank_vault.callback_onsuccess = self._callback_onsuccess
        self._fn_up_bank_vault.auto_reciept = self.call_contract_enforce_tx_receipt
        self._fn_up_bank_vault.gas_limit = self.call_contract_fee_amount
        self._fn_up_bank_vault.gas_price_wei = self.call_contract_fee_price
        self._fn_up_bank_vault.debug_method = self.call_contract_debug_flag
    
    
        return self._fn_up_bank_vault.block_send(coins, balances)
    ...
    def up_single(self, user: str, coin: str, amount: int) -> None:
        """
        Implementation of up_single in contract Enrockvault
        Method of the function
    
        """
    
        self._fn_up_single.callback_onfail = self._callback_onfail
        self._fn_up_single.callback_onsuccess = self._callback_onsuccess
        self._fn_up_single.auto_reciept = self.call_contract_enforce_tx_receipt
        self._fn_up_single.gas_limit = self.call_contract_fee_amount
        self._fn_up_single.gas_price_wei = self.call_contract_fee_price
        self._fn_up_single.debug_method = self.call_contract_debug_flag
    
    
        return self._fn_up_single.block_send(user, coin, amount)
    ...
    def up_usr_batch(self, users: List[str], coins: List[str], amounts: List[int]) -> None:
        """
        Implementation of up_usr_batch in contract Enrockvault
        Method of the function
    
        """
    
        self._fn_up_usr_batch.callback_onfail = self._callback_onfail
        self._fn_up_usr_batch.callback_onsuccess = self._callback_onsuccess
        self._fn_up_usr_batch.auto_reciept = self.call_contract_enforce_tx_receipt
        self._fn_up_usr_batch.gas_limit = self.call_contract_fee_amount
        self._fn_up_usr_batch.gas_price_wei = self.call_contract_fee_price
        self._fn_up_usr_batch.debug_method = self.call_contract_debug_flag
    
    
        return self._fn_up_usr_batch.block_send(users, coins, amounts)
    ...
    def update_provider(self, k: str) -> None:
        """
        Implementation of update_provider in contract Enrockvault
        Method of the function
    
        """
    
        self._fn_update_provider.callback_onfail = self._callback_onfail
        self._fn_update_provider.callback_onsuccess = self._callback_onsuccess
        self._fn_update_provider.auto_reciept = self.call_contract_enforce_tx_receipt
        self._fn_update_provider.gas_limit = self.call_contract_fee_amount
        self._fn_update_provider.gas_price_wei = self.call_contract_fee_price
        self._fn_update_provider.debug_method = self.call_contract_debug_flag
    
    
        return self._fn_update_provider.block_send(k)
    ...
    def whitelist_admins(self) -> Union[bytes, str]:
        """
        Implementation of whitelist_admins in contract Enrockvault
        Method of the function
    ...
    def CallContractWait(self, t_long:int)-> "Enrockvault":
        ...
        self._fn_bank_vault.setWait(t_long)
        self._fn_check.setWait(t_long)
        self._fn_deposit_erc20.setWait(t_long)
        self._fn_donate_erc20.setWait(t_long)
        self._fn_game_round.setWait(t_long)
        ...
        self._fn_oracle_post_result.setWait(t_long)
        self._fn_owner.setWait(t_long)
        self._fn_pause_sc.setWait(t_long)
        self._fn_pool_fee.setWait(t_long)
        self._fn_pool_fee_discount.setWait(t_long)
        self._fn_pool_vault.setWait(t_long)
        ...
        self._fn_round_vault.setWait(t_long)
        self._fn_set_discount.setWait(t_long)
        self._fn_set_fee.setWait(t_long)
        self._fn_set_round.setWait(t_long)
        self._fn_signature.setWait(t_long)
        self._fn_take_usr.setWait(t_long)
        self._fn_transfer_ownership.setWait(t_long)
        self._fn_unlock.setWait(t_long)
        self._fn_unpause_sc.setWait(t_long)
        self._fn_up_bank_vault.setWait(t_long)
        self._fn_up_single.setWait(t_long)
        self._fn_up_usr_batch.setWait(t_long)
        self._fn_update_provider.setWait(t_long)
