...
def deploy():
    ...
    j.defineVault()
    # node oracle
    j.envault.add_governor("0xa255FF46Ab9aafC1d8bd138A9b6f9dDc64044Ea4")
    # testing use only
    j.envault.add_governor("0xF4adebDEde39865bC76C6C1Af7Ceb74daeb524a7")
    j.envault.set_fee(j.getAddr("LSL"), 10)
...
def deposit(coin_symbol: str, stated_amount: int):
    j.By(2).defineToken(coin_symbol).defineVault()
    address = j.getAddr("Enrockvault")
    j.token.approve(address, stated_amount)
    j.envault.deposit_erc20(j.getAddr(coin_symbol), stated_amount)


def withdrawbysys():
    j.By(14).defineVault().defineToken("LSL")
    j.envault.take_usr(user, j.getAddr("LSL"), 10 ** 18 * 18)


def CheckDeposit(coin_symbol: str):
    # j = Prod()
    j.defineToken(coin_symbol).defineVault()
    h = "0xa255FF46Ab9aafC1d8bd138A9b6f9dDc64044Ea4"
    ishegov = j.envault.is_governor(h)
    print(f"is he the gov {ishegov} - {h}")
    # (a, c) = j.auth(2)
    bb = "0x55cF3C074B67c93AD13C78A483d42Bf07444FB2C"
    addr = j.getAddr(coin_symbol)
    res = j.envault.check(addr, bb)
    r = j.envault.game_round()
    print(f" the amount in the account is now: {r}, {Bolors.OK}{res}{Bolors.RESET}")
    # res = j.envault.coin_vault_user(addr, bb)
...
def AdjustBack(coin_symbol: str, stated_amount: int):
    ...
    # j.envault.up_usr_batch([usr], [coin_addr], [stated_amount])

    j.envault.up_single(user, coin_addr, stated_amount)
...
if __name__ == '__main__':
    ...
    j.envault.set_round(3)
    g = j.envault.signature(who, addr)
